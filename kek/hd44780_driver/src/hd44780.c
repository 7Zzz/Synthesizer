...
static void set_output(const bool output)
{
	...
	if (output)
  {
	...
	GPIO_Init(Lcd_Conf.port, &GPIO_InitStruct);
}
...
static void enable(const bool pulse)
{
	if (pulse)
  {
		GPIO_ToggleBits(Lcd_Conf.port, Lcd_Conf.e);
		delay(150);
	}
	GPIO_ToggleBits(Lcd_Conf.port, Lcd_Conf.e);
	delay(150);
...
static void write(const u8 data, const bool reg)
{
	...
	GPIO_ResetBits(Lcd_Conf.port, Lcd_Conf.rw);
	GPIO_WriteBit (Lcd_Conf.port, Lcd_Conf.rs, !reg);
	GPIO_WriteBit (Lcd_Conf.port, Lcd_Conf.db7, (data & 0x8) >> 3);
	GPIO_WriteBit (Lcd_Conf.port, Lcd_Conf.db6, (data & 0x4) >> 2);
	GPIO_WriteBit (Lcd_Conf.port, Lcd_Conf.db5, (data & 0x2) >> 1);
	GPIO_WriteBit (Lcd_Conf.port, Lcd_Conf.db4, (data & 0x1));
	enable(true);
}
...
static bool read_busy(void)
{
	...
	set_output(false);
	GPIO_ResetBits(Lcd_Conf.port, Lcd_Conf.rs | Lcd_Conf.db7);
	GPIO_SetBits(Lcd_Conf.port, Lcd_Conf.rw);
	enable(false);
	data = GPIO_ReadInputDataBit(Lcd_Conf.port, Lcd_Conf.db7);
	enable(false);
	enable(true);
...
static void task_add(const hd44780_command_t command, const bool reg,	const u16 data, u8 nibbles)
{
		...
		const hd44780_lines_t lines, const hd44780_font_t font)
{
	...
	task_add(HD44780_WAIT, NONE, 1, NONE);
	task_add(HD44780_WRITE, true,	HD44780_FUNCTION | HD44780_4BIT | Lcd_Conf.lines | Lcd_Conf.font,	2);
	task_add(HD44780_WAIT, NONE, 1, NONE);
